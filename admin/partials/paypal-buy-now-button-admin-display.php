...
/**
 * @class       class MBJ_PayPal_Buy_Now_Button_Admin_Display {
 * @version	1.0.0
...
class MBJ_PayPal_Buy_Now_Button_Admin_Display {
    ...
    public static function add_settings_menu() {
        ...
        add_options_page('Paypal Buy Now Button Options', 'Paypal Buy Now Button', 'manage_options', 'paypal-buy-now-button', array(__CLASS__, 'paypal_buy_now_button_options'));
    }
     ...
     * @access   public
     */
    public static function paypal_buy_now_button_options() {
        $setting_tabs = apply_filters('paypal_buy_now_button_options_setting_tab', array('general' => 'General', 'help' => 'Help'));
        $current_tab = (isset($_GET['tab'])) ? $_GET['tab'] : 'general';
        ?>
        <h2 class="nav-tab-wrapper">
            <?php
            foreach ($setting_tabs as $name => $label)
                echo '<a href="' . admin_url('admin.php?page=paypal-buy-now-button&tab=' . $name) . '" class="nav-tab ' . ( $current_tab == $name ? 'nav-tab-active' : '' ) . '">' . $label . '</a>';
            ?>
        </h2>
        <?php
        foreach ($setting_tabs as $setting_tabkey => $setting_tabvalue) {
            switch ($setting_tabkey) {
                case $current_tab:
                    do_action('paypal_buy_now_button_' . $setting_tabkey . '_setting_save_field');
                    do_action('paypal_buy_now_button_' . $setting_tabkey . '_setting');
                    break;
            }
        }
    }
    ...
    public static function paypal_button_shortcode($atts, $content = null) {
        ...
        $output .= '<input type="hidden" name="business" value="' . esc_attr($paypal_buy_now_button_bussiness_email) . '">';
        ...
        if (isset($paypal_buy_now_button_currency) && !empty($paypal_buy_now_button_currency)) {
            $output .= '<input type="hidden" name="currency_code" value="' . esc_attr($paypal_buy_now_button_currency) . '">';
        }
        ...
        if (isset($paypal_buy_now_button_return_page) && !empty($paypal_buy_now_button_return_page)) {
            $paypal_buy_now_button_return_page = get_permalink($paypal_buy_now_button_return_page);
            $output .= '<input type="hidden" name="return" value="' . esc_url($paypal_buy_now_button_return_page) . '">';
        }

        $output .= '<input type="image" name="submit" border="0" src="' . esc_url($paypal_buy_now_button_custom_button) . '" alt="PayPal - The safer, easier way to pay online">';
        $output .= '</form></div>';
